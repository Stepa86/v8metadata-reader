...
Процедура ОпределитьТипВыгрузки()
	...
	_ЭтоВыгрузкаКонфигуратора = файл.Существует() И файл.ЭтоФайл();
	
	_лог.Отладка("Это выгрузка конфигурации: " + _ЭтоВыгрузкаКонфигуратора );
	
	_ФайлКонфигурацииЕДТ = ОбъединитьПути( _КаталогИсходников, "Configuration", "Configuration.mdo" );
	
	файл = Новый Файл(_ФайлКонфигурацииЕДТ);
	
	_ЭтоВыгрузкаЕДТ = файл.Существует() И файл.ЭтоФайл();
