...
Функция ПрочитатьИнформациюОПоддержке( ПутьКПроекту )
	...
	Если ОбщегоНазначения.ЭтоВыгрузкаКонфигурации( ПутьКПроекту ) Тогда

		путьКФайлуПоддержки = ОбъединитьПути( ПутьКПроекту, "Ext", "ParentConfigurations.bin" );

	ИначеЕсли ОбщегоНазначения.ЭтоВыгрузкаЕДТ( ПутьКПроекту ) Тогда
	...
	Если ОбщегоНазначения.ФайлСуществует( путьКФайлуПоддержки ) Тогда
