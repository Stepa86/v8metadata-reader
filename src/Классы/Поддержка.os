...
Функция ПолучитьДанныеПоддержки(Знач пПутьКФайлуПоддержки)
	...
	Для цНомерКонфигурации = 1 По всегоКонфигураций Цикл
		...
		поставка.Вставить("Версия", данные[сдвиг + ид_ВерсияКонфигурации]);
		поставка.Вставить("Поставщик", данные[сдвиг + ид_ПоставщикКонфигурации]);
		поставка.Вставить("Конфигурация", данные[сдвиг + ид_Конфигурация]);
		...
		Для цНомерОбъекта = 0 По всегоОбъектов - 1 Цикл
			...
			сдвигОбъекта = сдвиг + началоБлокаОбъектов + цНомерОбъекта * полейДляОбъекта;
