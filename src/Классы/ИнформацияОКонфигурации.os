...
Функция ОбъектыПодсистем(Знач пПодсистемы,
		Знач пПодсистемыСПодчиненными = Неопределено,
		Знач пПодсистемыСРодителями = Неопределено) Экспорт
	
	составПодсистем = ПолучитьСоставПодсистем();
	
	соотОтобранныеОбъекты = Новый Соответствие;
	
	Для каждого цПодсистема Из пПодсистемы Цикл
		
		объекты = ЗначениеСтруктуры(составПодсистем, цПодсистема + "._Объекты_");
		
		СкопироватьСоответствие(соотОтобранныеОбъекты, объекты);
		
		_лог.Информация("Для %1 добавлено %2 объектов", цПодсистема, объекты.Количество());

	КонецЦикла;
	
	Для каждого цПодсистема Из ОбеспечитьМассив(пПодсистемыСПодчиненными) Цикл
		
		объекты = ЗначениеСтруктуры(составПодсистем, цПодсистема + "._ОбъектыСПодчиненными_");
	...
	Для каждого цПодсистема Из ОбеспечитьМассив(пПодсистемыСРодителями) Цикл
		
		объекты = ЗначениеСтруктуры(составПодсистем, цПодсистема + "._ОбъектыСРодителями_");
	...
	Для каждого цОбъект Из объекты Цикл
...
Функция ПрочитатьСоставПодсистемыEDT(Знач пИмяПодсистемы, Знач пТекущийКаталогПодсистемы)
	...
	Для каждого цЭлементФайла Из ОбеспечитьМассив(объектыФайла) Цикл
		...
		Для каждого цЭлемент Из цЭлементФайла Цикл
			...
			Если цЭлемент.Ключ = "content" Тогда
